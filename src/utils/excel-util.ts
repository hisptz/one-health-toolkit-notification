...
import { uniq, flattenDeep, keys, map } from 'lodash';
...
export class ExcelUtil {
  ...
  async writeToSingleSheetExcelFile(
    ...
    try {
      ...
      const ws = xlsx.utils.json_to_sheet(jsonData, {
        header: uniq(flattenDeep(map(jsonData, (data) => keys(data)))),
        skipHeader
      });
  ...
  async writeToMultipleSheetExcelFile(jsonDataObject: any, skipHeader = false) {
    try {
      ...
      let workbook = xlsx.utils.book_new();
      for (const sheetName of keys(jsonDataObject)) {
    ...
    try {
      ...
      for (const sheet_name of sheet_name_list) {
        try {
          ...
          data[`${sheet_name}`.trim()] = map(dataObjects, (dataObj: any) => {
            const newDataObj: any = {};
            for (const key of keys(dataObj)) {
